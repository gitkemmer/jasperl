...
package JasPerl::JSTL::Core::ConditionalTagSupport;
...
sub do_tag {
    ...
    if (defined (my $var = $self->_var)) {
        my $ctx = $self->get_jsp_context();
        my $value = JasPerl::Type::Boolean->value_of($cond);
        $ctx->set_attribute($var, $value, $self->_scope);
    }

    # FIXME: var visible in body?
...
JasPerl::JSTL::Core::ConditionalTagSupport - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::JSTL::Core::ConditionalTagSupport;
