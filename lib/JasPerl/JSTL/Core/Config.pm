...
package JasPerl::JSTL::Core::Config;
...
use constant {
    FMT_FALLBACK_LOCALE      => 'javax.servlet.jsp.jstl.fmt.fallbackLocale',
    FMT_LOCALE               => 'javax.servlet.jsp.jstl.fmt.locale',
    FMT_LOCALIZATION_CONTEXT => 'javax.servlet.jsp.jstl.fmt.localizationContext',
    FMT_TIME_ZONE            => 'javax.servlet.jsp.jstl.fmt.timeZone',
    SQL_DATA_SOURCE          => 'javax.servlet.jsp.jstl.sql.dataSource',
    SQL_MAX_ROWS             => 'javax.servlet.jsp.jstl.sql.maxRows'
};

sub find {
    return $_[1]->find_attribute($_[2]);
}

sub get {
    my ($class, $object, $name, $scope) = @_;

    if (defined $scope) {
        return $object->get_attribute($name, $scope);
    } else {
...
sub set {
    my ($class, $object, $name, $value, $scope) = @_;

    if (defined $scope) {
        return $object->set_attribute($name, $value, $scope);
    } else {
...
sub remove {
    ...
    if (defined $scope) {
        return $object->remove_attribute($name, $scope);
    } else {
...
JasPerl::JSTL::Core::Config - <One line description of module's purpose>
