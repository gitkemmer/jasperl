...
package JasPerl::JSTL::SQL::ResultSupport;
...
sub fetch_result {
    my ($class, $sth, $max_rows) = @_;
    my $names = $sth->{NAME};
    # my $types = $sth->{TYPE}; # per-column type info; wrap rows?
    my $rows = $sth->fetchall_arrayref([ ], $max_rows);

    return JasPerl::JSTL::SQL::Result->new(
        columnNames => $names,
        rowsByIndex => $rows,
        # FIXME: easier way?
        limitedByMaxRows => defined $max_rows && @{$rows} == $max_rows
    );
}
...
JasPerl::JSTL::SQL::ResultSupport - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::JSTL::SQL::ResultSupport;
