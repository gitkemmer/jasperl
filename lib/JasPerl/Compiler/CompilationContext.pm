...
package JasPerl::Compiler::CompilationContext;
...
use constant {
    ...
    COMPILER    => 'jasperl.compiler.jspCompiler',
...
# FIXME: root action
use constant JSP_VERSION => "jasperl.compiler.jspVersion";

my @SCOPES = ( FILE_SCOPE, MODULE_SCOPE, APPLICATION_SCOPE );

use JasPerl::Util::Bean; # FIXME: Role!

with qw(JasPerl::JspContext);

has [ qw(jspCompiler module) ] => ( is => 'rwp' );

has [ qw(jspConfig runtimeContext) ] => ( is => 'lazy', clearer => 1 );
...
sub _build__attributes {
    my $self = shift;
    return JasPerl::Compiler::CompilationContext::Attributes->new(
        APPLICATION() => $self->get_runtime_context(),
        COMPILER()    => $self->get_jsp_compiler(),
...
sub _build_runtime_context {
    return $_[0]->get_jsp_compiler()->get_context();
}
...
sub include {
    ...
    my $compiler = $self->get_jsp_compiler($path);#???
...
package # hide from PAUSE
    JasPerl::Compiler::CompilationContext::Attributes;
...
JasPerl::Compiler::CompilationContext - <One line description of module's purpose>
