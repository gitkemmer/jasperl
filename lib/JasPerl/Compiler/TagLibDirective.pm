...
package JasPerl::Compiler::TagLibDirective;
...
has prefix => ( required => 1 );

has [ qw(tagdir uri) ] => ( predicate => 1 );
...
sub do_tag {
    ...
    my $uri = $self->get_uri()
        or die "$self: tagdir not supported (yet)!"; # TODO

    if (my $taglib = JasPerl::TagExt::TagLibrary->get_tag_library($uri)) {
        $ctx->add_tag_library($self->get_prefix(), $taglib);
    } else {
        die "$self: taglib $uri not found";
    }
}
...
JasPerl::Compiler::TagLibDirective - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::Compiler::TagLibDirective;
