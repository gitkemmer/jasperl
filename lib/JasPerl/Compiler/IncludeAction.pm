...
package JasPerl::Compiler::IncludeAction;
...
has flush => ( coerce => sub { JasPerl::Type::Boolean->parse($_[0]) });
...
sub do_tag {
    ...
    my $out = $ctx->get_out();
    $out->print($ctx->get_context_var(), '->include(', quote($uri));
    $out->print(', ', $self->get_flush()) if $self->get_flush();
    $out->println(');');
}
...
JasPerl::Compiler::IncludeAction - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::Compiler::IncludeAction;
