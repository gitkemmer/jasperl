...
has parent => ( is => 'rw', weak_ref => 1, init_arg => undef );

has jspContext => ( is => 'rw', init_arg => undef );

sub invoke {
    my ($self, $fragment, $out) = @_;

    if ($out) {
        $self->get_jsp_context()->push_body($out);

        my $ok = eval { $self->$fragment(); 1 };

        my $e = $@;
        $self->get_jsp_context()->pop_body();
        die $e unless $ok;
    } else {
        $self->$fragment();
    }
...
sub find_ancestor_with_class {
    my ($tag, $class) = @_;
    while ($tag = $tag->get_parent()) {
        return $tag if $tag->DOES($class);
    }
    return $tag;
}
...
JasPerl::TagExt::JspTag - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::TagExt::JspTag;
