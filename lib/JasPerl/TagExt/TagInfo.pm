...
package JasPerl::TagExt::TagInfo;

use JasPerl::TagExt::TagAttributeInfo;
...
use constant {
    ...
    BODY_CONTENT_SCRIPTLESS    => 'scriptless',
    BODY_CONTENT_TAG_DEPENDENT => 'tagdependent'
};

has [ qw(name class) ] => ( is => 'ro', required => 1 );

has attributes => (
    is => 'ro',
    default => sub { [ ] },
    coerce => sub{
        [ map { JasPerl::TagExt::TagAttributeInfo->new($_) } @{$_[0]} ]
    }
);

has bodyContent => ( is => 'ro', default => BODY_CONTENT_SCRIPTLESS );

has dynamicAttributes => ( is => 'ro', reader => 'has_dynamic_attributes' );

has _attribute_map => ( is => 'lazy' );

sub _build__attribute_map {
    return { map { $_->get_name() => $_ } @{$_[0]->get_attributes()} };
}

sub get_attribute {
    return $_[0]->_attribute_map->{$_[1]};
}
...
JasPerl::TagExt::TagInfo - <One line description of module's purpose>
