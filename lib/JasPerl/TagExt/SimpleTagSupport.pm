...
package JasPerl::TagExt::SimpleTagSupport;
...
has jspBody => ( is => 'rw', init_arg => undef );

sub do_body {
    my ($self, $out) = @_;
    if (my $body = $self->get_jsp_body()) {
        $self->invoke($body, $out);
    }
}

use constant BODY_CONTENT => 'scriptless';
...
JasPerl::TagExt::SimpleTagSupport - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::TagExt::SimpleTagSupport;
