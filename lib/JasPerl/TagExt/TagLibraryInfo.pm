...
package JasPerl::TagExt::TagLibraryInfo;
...
has [ qw(tags functions) ] => ( is => 'ro', default => sub { [ ] } );

has [ qw(_function_map _tag_map) ] => ( is => 'lazy' );

sub _build__tag_map {
    return { map { $_->get_name() => $_ } @{$_[0]->get_tags()} };
}

sub _build__function_map {
    return { map { $_->get_name() => $_ } @{$_[0]->get_functions()} };
}

sub get_tag {
    return $_[0]->_tag_map->{$_[1]};
}

sub get_function {
    return $_[0]->_function_map->{$_[1]};
}
...
JasPerl::TagExt::TagLibraryInfo - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::TagExt::TagLibraryInfo;
