...
has _init_params => ( is => 'rw' );

sub init {
    shift->_init_params({ @_ });
}

sub service {
    shift->_jsp_service(@_);
}

sub get_init_parameter {
    return $_[0]->_init_params->{$_[1]};
}

sub get_init_parameter_names {
    return JasPerl::Util::Enumeration->from_list(keys %{$_[0]->_init_params});
}
...
JasPerl::JspPage - <One line description of module's purpose>
