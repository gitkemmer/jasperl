...
package JasPerl::Util::Cloneable;
...
my %CLONE_METHODS = (
    ...
    'HASH'   => sub { return { %{$_[0]} } },
    'SCALAR' => sub { my $val = ${$_[0]}; return \$val }
);

sub clone {
    my $self = shift;
    my $class = ref $self;
    if (my $method = $CLONE_METHODS{reftype($self)}) {
        return bless $self->$method(), $class;
    } else {
        JasPerl::Util::CloneNotSupportedException->throw();
    }
}
...
JasPerl::Util::Cloneable - <One line description of module's purpose>

=head1 SYNOPSIS

use JasPerl::Util::Cloneable;
