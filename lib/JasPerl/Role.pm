...
BEGIN { *INFO = \%Moo::Role::INFO }

sub _accessor_maker_for {
    my ($class, $target) = @_;

    # copied from Moo::Role::import
    return $INFO{$target}{accessor_maker} ||= do {
...
sub import {
    ...
    unless ($INFO{$target} && $INFO{$target}{is_role}) {
        Moo::Role->import::into($target);
    }

    Moo::Role->apply_roles_to_object(
        $class->_accessor_maker_for($target),
        'Method::Generate::Accessor::Role::JasPerl::Role'
    );
}
