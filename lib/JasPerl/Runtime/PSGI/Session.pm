...
package JasPerl::Runtime::PSGI::Session;

# VERSION

use JasPerl::Util::Bean;
with qw(JasPerl::Session);

has _env => ( is => 'ro', init_arg => 'env' );

sub _build_attributes {
    return $_[0]->_env->{'psgix.session'} ||= { };
}

sub _build_id {
    return $_[0]->_env->{'psgix.session.options'}->{id};
}

sub invalidate {
    $_[0]->_env->{'psgix.session.options'}->{expire} = 1;
}
...
JasPerl::Runtime::PSGI::Session - <One line description of module's purpose>
