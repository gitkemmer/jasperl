...
has maxAge => ( is => 'rw', default => -1 );

# FIXME: version = 0/1?
has [ qw(comment domain path version) ] => ( is => 'rw' );

has [ qw(httpOnly secure) ] => ( is => 'rw', boolean => 1 );

# FIXME
sub BUILDARGS {
    my ($class, @args) = @_;
    return { name => $args[0], value => $args[1] } if @args == 2;
    return { @args };
}
...
JasPerl::Cookie - <One line description of module's purpose>
